...
def moltoimage(smiles):
    ...
    return Chem.Draw.MolToImage(mol, size=(600,250))
...
def ld50_scale(pred, max = 3, min = -5):
    '''
...
test_exp = get_data('../data/train_test_sets/test_labels')
avg_preds = get_data('../results/avg_predictions/App_zones')
H_avg_preds = get_data('../results/avg_predictions/Hmodel_avgp')
B_avg_preds = get_data('../results/avg_predictions/Bmodel_avgp')

#model comparsion
reg_models_result = get_data('../results/model_evaluation/reg_scores')
toxic_models_result = get_data('../results/model_evaluation/toxic_scores')
epa_models_result = get_data('../results/model_evaluation/epa_scores')

#data for chemical space
tsne = get_data('../results/chemical_space/test_tsne')
svgs = get_mol_svg('../results/chemical_space/test_tsne')
...
if st.sidebar.checkbox('Chemical Space'):
    st.write('Only The Test Set is shown here due to the limitation of streamlit.')
    st.write('Add color to the points are under developing.')
